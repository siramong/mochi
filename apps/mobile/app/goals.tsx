...
export function GoalsScreen() {
  ...
  const handleCreateGoal = async () => {
    ...
    try {
      ...
      showAlert({
        title: 'Error',
        message: err instanceof Error ? err.message : 'No se pudo crear la meta',
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-purple-50">
        ...
        <Text className="mt-3 text-sm font-semibold text-purple-700">Cargando metas...</Text>
      </SafeAreaView>
    )
  }
    ...
    <>
      <SafeAreaView className="flex-1 bg-purple-50">
        <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false}>
          ...
          <View className="mt-6">
            <Text className="text-2xl font-extrabold text-purple-900">Metas</Text>
          ...
          {celebratingGoal && (
            <View className="mt-5 items-center rounded-3xl border-2 border-green-200 bg-green-50 p-5">
              ...
              >
                <Text className="font-bold text-green-800">Seguir con mis metas</Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          <View className="mt-5">
            {goals.length === 0 ? (
              ...
              goals.map((goal) => (
                <GoalCard key={goal.id} goal={goal} onPress={() => openEditSheet(goal)} />
              ))
            )}
          </View>
      ...
      <Modal transparent visible={showCreateSheet} animationType="slide" onRequestClose={closeCreateSheet}>
              ...
              <View className="rounded-t-3xl bg-white px-5 pb-8 pt-5">
                ...
                <View className="mt-4">
                  ...
                  <TextInput
                    ...
                    onChangeText={(value) => setGoalDraft((prev) => ({ ...prev, title: value }))}
                    editable={!creating}
                  />
                </View>
                    ...
                    {goalColors.map((color) => {
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          onPress={() => setGoalDraft((prev) => ({ ...prev, color: color.key }))}
                          disabled={creating}
                        >
      ...
      <Modal transparent visible={showEditSheet} animationType="slide" onRequestClose={closeEditSheet}>
                ...
                <View className="mt-5">
                  ...
                  <View className="mt-3 flex-row items-center justify-between rounded-2xl bg-purple-50 px-3 py-3">
                    <TouchableOpacity
                      ...
                      onPress={() => setEditProgress((prev) => clampProgress(prev - 10))}
                      disabled={savingEdit}
                    >
                      <Ionicons name="remove" size={18} color="#7c3aed" />
                    </TouchableOpacity>

                    <Text className="text-xl font-extrabold text-purple-900">{progressLabel}</Text>
