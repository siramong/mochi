...
export function GratitudeScreen() {
  ...
  const loadGratitudeData = useCallback(async () => {
    ...
    try {
      ...
      const { data: todayData, error: todayError } = await supabase
        .from('gratitude_logs')
        .select('id, user_id, entry_1, entry_2, entry_3, logged_date, created_at')
        .eq('user_id', userId)
      ...
      setError(err instanceof Error ? err.message : 'No se pudo cargar tu diario de gratitud')
      setTodayEntry(null)
      setHistory([])
    } finally {
      setLoading(false)
  ...
  const handleSaveGratitude = async () => {
    ...
    try {
      ...
      const todayISO = getTodayISO()
      const { error: upsertError } = await supabase.from('gratitude_logs').upsert(
        {
          user_id: userId,
          entry_1: entry1.trim(),
      ...
      showAlert({
        title: 'Error',
        message: err instanceof Error ? err.message : 'No se pudo guardar tu registro',
      ...
      <SafeAreaView className="flex-1 bg-emerald-50">
        <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false}>
          ...
          <View className="mt-6 rounded-3xl border-2 border-emerald-200 bg-white p-4">
            <Text className="text-2xl font-extrabold text-emerald-900">Gratitud</Text>
          ...
          {loading ? (
            <View className="mt-6 items-center rounded-3xl border-2 border-emerald-200 bg-white p-6">
              ...
              <Text className="mt-3 text-sm font-semibold text-emerald-800">Cargando diario...</Text>
            </View>
          ) : error ? (
            ...
            <>
              <View className="mt-6 rounded-3xl border-2 border-emerald-200 bg-white p-4">
                <View className="flex-row items-center justify-between">
                  <Text className="text-lg font-extrabold text-emerald-900">Registro de hoy</Text>
                  {todayEntry && !editingToday && (
                    <TouchableOpacity
                      className="rounded-full bg-emerald-100 px-3 py-1"
                      onPress={() => setEditingToday(true)}
                    >
                      <Text className="text-xs font-extrabold text-emerald-800">Editar</Text>
                    </TouchableOpacity>
                  )}
                </View>
                ...
                {showEditor ? (
                  <>
                    <View className="mt-4">
                      <Text className="mb-2 text-sm font-bold text-emerald-900">Cosa 1</Text>
                      <TextInput
                    ...
                    <View className="mt-4">
                      <Text className="mb-2 text-sm font-bold text-emerald-900">Cosa 2 (opcional)</Text>
                      <TextInput
                        className="rounded-2xl border-2 border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-slate-800"
                    ...
                    <View className="mt-4">
                      <Text className="mb-2 text-sm font-bold text-emerald-900">Cosa 3 (opcional)</Text>
                      <TextInput
                        className="rounded-2xl border-2 border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-slate-800"
                  ...
                  <View className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
                    <Text className="text-sm font-semibold text-emerald-900">1. {todayEntry.entry_1}</Text>
                    {todayEntry.entry_2 ? (
                      <Text className="mt-2 text-sm font-semibold text-emerald-900">2. {todayEntry.entry_2}</Text>
                    ) : null}
                    {todayEntry.entry_3 ? (
                      <Text className="mt-2 text-sm font-semibold text-emerald-900">3. {todayEntry.entry_3}</Text>
                    ) : null}
                  </View>
                )}
              </View>
              ...
              <View className="mt-6">
                <Text className="text-lg font-extrabold text-emerald-900">Últimos 5 registros</Text>
                  ...
                  <View className="mt-3">
                    {history.map((entry) => (
                      ...
                      >
                        ...
                        <Text className="mt-2 text-sm font-semibold text-emerald-900">1. {entry.entry_1}</Text>
                        {entry.entry_2 ? (
                          <Text className="mt-1 text-sm font-semibold text-emerald-900">2. {entry.entry_2}</Text>
                        ) : null}
                        {entry.entry_3 ? (
                          <Text className="mt-1 text-sm font-semibold text-emerald-900">3. {entry.entry_3}</Text>
                        ) : null}
                      </View>
                    ))}
                  </View>
