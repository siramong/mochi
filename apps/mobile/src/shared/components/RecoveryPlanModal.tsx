        ...
        <View className="bg-white rounded-3xl p-6 w-full max-w-sm">
          ...
          <ScrollView className="mb-4 max-h-64">
            {plan.recovery_tasks.map((task, index) => (
              <View key={index} className="mb-3 p-3 bg-gradient-to-r from-pink-50 to-purple-50 rounded-xl border border-pink-200">
                <View className="flex-row items-start gap-2">
                  <View className="bg-purple-500 rounded-full w-8 h-8 items-center justify-center mt-0.5">
                    <Text className="text-white font-bold text-sm">{task.day}</Text>
                  </View>
          ...
          <View className="bg-yellow-50 rounded-xl p-3 mb-4 border border-yellow-200">
            <Text className="text-xs text-yellow-800">
              <Text className="font-semibold">Pista:</Text> Completa estos 3 días
              y tu racha estará de vuelta. Tú puedes, amiga 💪
            </Text>
          </View>
