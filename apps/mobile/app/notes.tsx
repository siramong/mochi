...
export function NotesScreen() {
  ...
  const orderedNotes = useMemo(
    () => [...notes].sort((a, b) => Number(b.is_pinned) - Number(a.is_pinned) || b.updated_at.localeCompare(a.updated_at)),
    [notes]
  )
  ...
  const openCreate = async () => {
    ...
    const { data, error: createError } = await supabase
      .from('quick_notes')
      .insert({ user_id: userId, title: null, content: ' ', color: 'yellow', is_pinned: false })
      .select('*')
      .single<QuickNote>()
  ...
  const togglePin = async () => {
    ...
    const { data } = await supabase
      .from('quick_notes')
      .update({ is_pinned: !activeNote.is_pinned, updated_at: new Date().toISOString() })
      .eq('id', activeNote.id)
      .eq('user_id', userId)
      .select('*')
      .single<QuickNote>()
  ...
  const deleteNote = async () => {
    ...
    await supabase.from('quick_notes').delete().eq('id', activeNote.id).eq('user_id', userId)
    ...
    <SafeAreaView className="flex-1 bg-yellow-50">
      <View className="flex-1 px-5">
        <View className="mt-4 flex-row items-center justify-between">
          <Text className="text-2xl font-extrabold text-yellow-900">Notas rápidas</Text>
          <TouchableOpacity className="h-11 w-11 items-center justify-center rounded-full bg-yellow-300" onPress={() => { void openCreate() }}>
            <Ionicons name="add" size={24} color="#854d0e" />
          </TouchableOpacity>
        </View>

        {loading ? <Text className="mt-4 text-sm font-semibold text-yellow-700">Cargando notas...</Text> : null}
            ...
            {orderedNotes.map((note) => (
              ...
              >
                <View className="flex-row items-start justify-between">
                  <Text className="flex-1 text-sm font-bold text-slate-900" numberOfLines={1}>{note.title || 'Sin título'}</Text>
                  {note.is_pinned ? <Ionicons name="pin" size={14} color="#7e22ce" /> : null}
                </View>
                <Text className="mt-2 text-xs font-semibold text-slate-700" numberOfLines={3}>{note.content.trim()}</Text>
                <View className="mt-2 h-2 w-2 rounded-full" style={{ backgroundColor: colorMap[note.color].dot }} />
              </TouchableOpacity>
            ))}
          </View>
        </ScrollView>
        ...
        <View className="flex-1 justify-end bg-black/25">
          <View className="rounded-t-3xl bg-white p-4">
            ...
            <View className="mt-3 flex-row items-center">
              {(['yellow', 'pink', 'blue', 'teal', 'purple'] as const).map((option) => (
                <TouchableOpacity
                  key={option}
      ...
      <Modal visible={optionsVisible} animationType="fade" transparent>
        <View className="flex-1 items-center justify-center bg-black/25 px-6">
          <View className="w-full rounded-2xl bg-white p-4">
            <TouchableOpacity className="rounded-xl bg-violet-100 px-3 py-3" onPress={() => { void togglePin() }}>
              <Text className="text-center text-sm font-bold text-violet-800">
                {activeNote?.is_pinned ? 'Desfijar' : 'Fijar'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity className="mt-2 rounded-xl bg-red-100 px-3 py-3" onPress={() => { void deleteNote() }}>
              <Text className="text-center text-sm font-bold text-red-700">Eliminar</Text>
            </TouchableOpacity>
            <TouchableOpacity className="mt-2 rounded-xl bg-slate-100 px-3 py-3" onPress={() => setOptionsVisible(false)}>
              <Text className="text-center text-sm font-bold text-slate-700">Cerrar</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
