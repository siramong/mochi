...
export function CycleWidget({ cycleData, hasPermission, onRequestPermission }: CycleWidgetProps) {
      ...
      <View className="mt-4 rounded-3xl border-2 border-violet-200 bg-white p-4">
        <View className="flex-row items-start">
          ...
          <View className="ml-3 flex-1">
            <Text className="text-sm font-extrabold text-violet-900">Conecta Flo o tu app de ciclo</Text>
        ...
        <TouchableOpacity
          className="mt-3 self-start rounded-2xl border border-violet-300 bg-violet-100 px-4 py-2"
          onPress={() => {
            void onRequestPermission()
          }}
        >
          <Text className="text-xs font-extrabold text-violet-800">Conectar ahora</Text>
        </TouchableOpacity>
      </View>
    )
  }
  ...
  const progress = buildPhaseProgress(cycleData.dayOfCycle, cycleData.cycleLength)
    ...
    <View className="mt-4 rounded-3xl border-2 border-violet-200 bg-white p-4">
      <View className="flex-row items-start justify-between">
        <View className="flex-1 pr-3">
          <View className={`self-start rounded-full border px-3 py-1 ${personality.phaseBadgeClass}`}>
          ...
          <Text className="mt-3 text-sm font-extrabold text-violet-900">Día {cycleData.dayOfCycle} de tu ciclo</Text>

          <View className="mt-3 h-2 w-full overflow-hidden rounded-full bg-violet-100">
            <View className="h-2 rounded-full" style={{ width: `${progress}%`, backgroundColor: progressColor }} />
          </View>
      ...
      <View className="mt-3 rounded-2xl border border-violet-100 bg-violet-50 p-3">
        <Text className="text-xs font-semibold text-violet-800">{personality.generalNote}</Text>
      </View>
    </View>
  )
}
