...
export function StudyTimerScreen() {
  ...
  const [chatAttachment, setChatAttachment] = useState<StudyAttachment | null>(null)
  ...
  const summaryText = useMemo(() => summarizeSession(specificTopic), [specificTopic])
  ...
  async function pickImageFromCamera(onSelect: (value: StudyAttachment) => void) {
  ...
  async function pickImageFromGallery(onSelect: (value: StudyAttachment) => void) {
  ...
  async function handleAskCompanion() {
    const userText = question.trim() || (chatAttachment ? 'Ayúdame con este material adjunto.' : '')
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'assistant', content: answer || 'No pude responder por ahora.' }])
      setQuestion('')
      setChatAttachment(null)
    } catch (err) {
      setChatError(err instanceof Error ? err.message : 'No se pudo consultar a Mochi')
  ...
  if (error) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-purple-50 px-6">
        ...
        <TouchableOpacity className="mt-6 rounded-2xl bg-purple-500 px-6 py-3" onPress={() => router.back()}>
      ...
      <SafeAreaView className="flex-1 bg-purple-50 px-6">
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          <View className="mt-4 rounded-3xl border-2 border-purple-200 bg-white p-4">
            <Text className="text-xl font-extrabold text-purple-900">{block?.subject}</Text>
            <Text className="mt-1 text-xs font-semibold text-purple-600">
              {block?.start_time} - {block?.end_time} · {formatDurationMinutes(totalSeconds)} min
            </Text>
            {discipline ? (
              <View className="mt-3 self-start rounded-full bg-purple-100 px-3 py-1">
                <Text className="text-xs font-bold text-purple-700">Disciplina: {discipline}</Text>
              </View>
            ) : null}
          </View>
          ...
          {phase === 'setup' ? (
            <View className="mt-4 rounded-3xl border-2 border-purple-200 bg-white p-4">
              <Text className="text-sm font-bold text-purple-800">¿Qué vas a estudiar específicamente hoy?</Text>
              <TextInput
              ...
              <Text className="mt-4 text-xs font-bold uppercase text-purple-700">Adjunto opcional</Text>
              <View className="mt-2 flex-row flex-wrap gap-2">
                <TouchableOpacity className="rounded-xl bg-purple-100 px-3 py-2" onPress={() => void pickImageFromCamera(setSetupAttachment)}>
                  <Text className="text-xs font-bold text-purple-800">Cámara</Text>
                </TouchableOpacity>
                <TouchableOpacity className="rounded-xl bg-purple-100 px-3 py-2" onPress={() => void pickImageFromGallery(setSetupAttachment)}>
                  <Text className="text-xs font-bold text-purple-800">Galería</Text>
                </TouchableOpacity>
                <TouchableOpacity className="rounded-xl bg-purple-100 px-3 py-2" onPress={() => void pickDocument(setSetupAttachment)}>
                  <Text className="text-xs font-bold text-purple-800">Archivo</Text>
                </TouchableOpacity>
                {setupAttachment ? (
                  <TouchableOpacity className="rounded-xl bg-red-100 px-3 py-2" onPress={() => setSetupAttachment(null)}>
              ...
              {setupAttachment ? (
                <View className="mt-3 rounded-2xl border border-purple-200 bg-purple-50 p-3">
                  {setupAttachment.previewUri ? (
                    <Image source={{ uri: setupAttachment.previewUri }} className="h-24 w-full rounded-xl" resizeMode="cover" />
                  ) : (
                    <View className="h-16 items-center justify-center rounded-xl bg-purple-100">
                      <Text className="text-xs font-bold text-purple-700">{getAttachmentLabel(setupAttachment.mimeType)}</Text>
                    </View>
                  )}
                  <Text className="mt-2 text-xs font-semibold text-purple-700" numberOfLines={1}>
              ...
              >
                {planning ? (
                  <View className="flex-row items-center">
                    ...
                    <Text className="ml-2 font-bold text-white">Creando plan...</Text>
                  </View>
                ) : (
                  <Text className="font-bold text-white">Pedir plan a Mochi</Text>
                )}
              </TouchableOpacity>

              {planError ? <Text className="mt-2 text-xs font-semibold text-red-600">{planError}</Text> : null}

              {studyPlan ? (
                <View className="mt-4 rounded-2xl border border-purple-200 bg-purple-50 p-3">
                  <Text className="text-sm font-semibold leading-5 text-purple-900">{studyPlan}</Text>
                </View>
              ) : null}
            ...
            <>
              <View className="mt-4 rounded-3xl border-2 border-purple-200 bg-white p-4">
                <Text className="text-xs font-bold uppercase text-purple-700">Tema actual</Text>
                <Text className="mt-1 text-base font-extrabold text-purple-900">{specificTopic}</Text>
              </View>
              ...
              >
                <Text className="font-bold text-purple-700">{isCompleting ? 'Guardando...' : 'Marcar como completado'}</Text>
              </TouchableOpacity>
            </>
          )}
        </ScrollView>
        ...
        {phase === 'studying' ? (
          ...
          >
            ...
            <Text className="ml-2 text-sm font-bold text-white">Preguntarle a Mochi</Text>
          </TouchableOpacity>
        ) : null}
      </SafeAreaView>

      <Modal visible={showCompanion} transparent animationType="slide" onRequestClose={() => setShowCompanion(false)}>
        <View className="flex-1 justify-end bg-black/40">
          <View className="max-h-[90%] rounded-t-3xl bg-white px-5 pb-10 pt-5">
            ...
            <View className="flex-row items-center justify-between">
              <View>
                <Text className="text-base font-extrabold text-purple-900">Compañera de estudio</Text>
                <Text className="text-xs font-semibold text-purple-600" numberOfLines={1}>
                  {specificTopic}
                </Text>
              </View>
            ...
            <ScrollView className="mt-3 max-h-[45%]" showsVerticalScrollIndicator={false}>
              {messages.length === 0 ? (
                <View className="items-center rounded-2xl border border-purple-200 bg-purple-50 p-4">
                  ...
                  <Text className="mt-2 text-center text-xs font-semibold text-purple-700">
                    Estoy lista para ayudarte con teoría, ejercicios, dudas o resúmenes.
                  </Text>
                </View>
              ) : (
                messages.map((message, index) => (
                  ...
                  >
                    <Text className="text-sm font-semibold text-slate-800">{message.content}</Text>
                  </View>
                ))
              )}
              {asking ? (
                <View className="mb-2 flex-row items-center rounded-2xl bg-purple-100 p-3">
                  ...
                  <Text className="ml-2 text-xs font-semibold text-purple-700">Mochi está pensando...</Text>
                </View>
              ) : null}
            </ScrollView>

            <View className="mt-3 flex-row flex-wrap gap-2">
              <TouchableOpacity className="rounded-xl bg-purple-100 px-3 py-2" onPress={() => void pickImageFromCamera(setChatAttachment)}>
                <Text className="text-xs font-bold text-purple-800">Cámara</Text>
              </TouchableOpacity>
              <TouchableOpacity className="rounded-xl bg-purple-100 px-3 py-2" onPress={() => void pickImageFromGallery(setChatAttachment)}>
                <Text className="text-xs font-bold text-purple-800">Galería</Text>
              </TouchableOpacity>
              <TouchableOpacity className="rounded-xl bg-purple-100 px-3 py-2" onPress={() => void pickDocument(setChatAttachment)}>
                <Text className="text-xs font-bold text-purple-800">Archivo</Text>
              </TouchableOpacity>
              {chatAttachment ? (
                <TouchableOpacity className="rounded-xl bg-red-100 px-3 py-2" onPress={() => setChatAttachment(null)}>
                  <Text className="text-xs font-bold text-red-700">Quitar</Text>
                </TouchableOpacity>
              ) : null}
            </View>
            ...
            {chatAttachment ? (
              <View className="mt-2 rounded-xl border border-purple-200 bg-purple-50 px-3 py-2">
                <Text className="text-xs font-semibold text-purple-700" numberOfLines={1}>
                  {chatAttachment.name}
                </Text>
              </View>
            ) : null}
            ...
            {chatError ? <Text className="mt-2 text-xs font-semibold text-red-600">{chatError}</Text> : null}
