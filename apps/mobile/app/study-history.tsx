      ...
      <ScrollView className="flex-1 px-5" showsVerticalScrollIndicator={false}>
        ...
        <View className="mt-6 rounded-3xl border-2 border-blue-200 bg-white p-4">
          <View className="flex-row items-center">
            ...
            <Text className="ml-2 text-2xl font-extrabold text-blue-900">Historial de estudio</Text>
          </View>
          <Text className="mt-1 text-sm font-semibold text-blue-600">Todas tus sesiones completadas</Text>
        </View>

        {!loading && totalSessions > 0 && (
          <View className="mt-4 flex-row gap-3">
            <View className="flex-1 rounded-2xl border-2 border-purple-200 bg-purple-100 p-4">
              <Text className="text-3xl font-extrabold text-purple-900">{totalSessions}</Text>
            ...
            <View className="flex-1 rounded-2xl border-2 border-blue-200 bg-blue-100 p-4">
              ...
              <Text className="mt-1 text-xs font-bold text-blue-600">
                {totalHours > 0 && totalMinutes > 0 ? `${totalMinutes}min más` : 'estudiadas'}
              </Text>
            </View>
          </View>
        )}
        ...
        {loading ? (
          <View className="mt-12 items-center">
            ...
            <Text className="mt-4 text-sm font-semibold text-blue-700">Cargando historial...</Text>
          </View>
        ) : error ? (
                ...
                {group.sessions.map((studySession) => (
                  ...
                  >
                    ...
                    <View className="ml-3 flex-1">
                      <Text className="text-sm font-bold text-slate-800">{studySession.subject}</Text>
                    ...
                    <Text className="text-xs font-semibold text-blue-400">
                      {new Date(studySession.completed_at).toLocaleTimeString('es-ES', {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </Text>
