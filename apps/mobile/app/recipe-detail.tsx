...
export function RecipeDetailScreen() {
  ...
  const [activeSession, setActiveSession] = useState<RecipeCookSession | null>(null)
  ...
  const loadRecipe = useCallback(async () => {
    ...
    try {
      ...
      const [recipeRes, ingredientsRes, stepsRes, sessionRes] = await Promise.all([
        supabase
          .from('recipes')
          .select('*')
          .eq('id', recipeId)
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      setRecipe((prev) => prev ? { ...prev, personal_notes: notesInput.trim() || null } : prev)
    ...
    showAlert({
      ...
      buttons: [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: () => {
            void (async () => {
              await supabase.from('recipes').delete().eq('id', recipe.id).eq('user_id', userId)
              router.back()
            })()
          },
        },
      ...
      showAlert({
        ...
        buttons: [
          { text: 'Cancelar', style: 'cancel' },
          {
            ...
            style: 'destructive',
            onPress: () => router.push(`/recipe-player?recipeId=${recipe.id}&fresh=true`),
          },
          {
            text: 'Continuar',
            onPress: () => router.push(`/recipe-player?recipeId=${recipe.id}&sessionId=${activeSession.id}`),
          },
        ],
      })
      return
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-orange-50">
        ...
        <Text className="mt-3 text-sm font-semibold text-orange-700">Cargando receta...</Text>
      </SafeAreaView>
    )
  }
      ...
      <View className="flex-1 bg-orange-50">
        <SafeAreaView className="flex-1">
          ...
          <View className="mt-5 rounded-3xl border-2 border-orange-200 bg-white p-5">
            ...
            <View className="mt-4 flex-row flex-wrap gap-2">
              {totalTime > 0 && (
                <View className="flex-row items-center rounded-full bg-orange-100 px-3 py-1.5">
                  ...
                  <Text className="ml-1 text-xs font-bold text-orange-800">{totalTime} min total</Text>
                </View>
              )}
              {recipe.prep_time_minutes > 0 && (
                <View className="flex-row items-center rounded-full bg-amber-100 px-3 py-1.5">
                  ...
                  <Text className="ml-1 text-xs font-bold text-amber-800">{recipe.prep_time_minutes} min prep</Text>
                </View>
              )}
              {recipe.cook_time_minutes > 0 && (
                <View className="flex-row items-center rounded-full bg-red-100 px-3 py-1.5">
                  ...
                  <Text className="ml-1 text-xs font-bold text-red-800">{recipe.cook_time_minutes} min cocción</Text>
                </View>
              )}
              <View className={`rounded-full px-3 py-1.5 ${diff.className}`}>
                <Text className={`text-xs font-bold ${diff.textClass}`}>{diff.label}</Text>
              </View>
              {recipe.cuisine_type ? (
                <View className="rounded-full bg-purple-100 px-3 py-1.5">
                  <Text className="text-xs font-bold text-purple-800">{recipe.cuisine_type}</Text>
                </View>
              ) : null}
            </View>
            ...
            {recipe.tags.length > 0 && (
              <View className="mt-3 flex-row flex-wrap gap-1">
                {recipe.tags.map((tag) => (
                  <View key={tag} className="rounded-full border border-orange-200 px-2 py-0.5">
                    <Text className="text-xs font-semibold text-orange-600">{tag}</Text>
                  </View>
                ))}
              </View>
            )}
            ...
            <View className="mt-4 rounded-3xl border-2 border-amber-300 bg-amber-50 p-4">
              <View className="flex-row items-center">
                ...
                <Text className="ml-2 text-sm font-extrabold text-amber-900">
                  Sesión activa — paso {activeSession.last_step_completed + 1} de {steps.length}
                </Text>
              </View>
          ...
          <View className="mt-4 rounded-3xl border-2 border-orange-200 bg-white p-4">
            <Text className="text-sm font-bold text-orange-900">Porciones</Text>
            <View className="mt-3 flex-row items-center justify-between rounded-2xl bg-orange-50 px-4 py-3">
              ...
              <Text className="text-xl font-extrabold text-orange-900">{servings}</Text>
              <TouchableOpacity
                className="h-9 w-9 items-center justify-center rounded-full bg-white"
            ...
            {servings !== recipe.servings && (
              <Text className="mt-2 text-center text-xs font-semibold text-orange-500">
                Receta original para {recipe.servings} porciones — cantidades ajustadas
              </Text>
            )}
          </View>
          ...
          {ingredients.length > 0 && (
            <View className="mt-4 rounded-3xl border-2 border-orange-200 bg-white p-4">
              <View className="flex-row items-center">
                ...
                <Text className="ml-2 text-lg font-extrabold text-orange-900">Ingredientes</Text>
              </View>
              <View className="mt-3">
                {ingredients.map((ing, idx) => {
                  const scaled = ing.amount !== null
                    ...
                    >
                      ...
                      <View className="flex-1">
                        <Text className="text-sm font-bold text-slate-800">
                          {scaled ? `${scaled}${ing.unit ? ` ${ing.unit}` : ''} ` : ''}
                          <Text className="font-semibold">{ing.name}</Text>
                        </Text>
                        {ing.notes ? (
                          <Text className="mt-0.5 text-xs font-semibold text-orange-600">{ing.notes}</Text>
                        ) : null}
                      </View>
                    </View>
                  )
          ...
          {steps.length > 0 && (
            <View className="mt-4 rounded-3xl border-2 border-orange-200 bg-white p-4">
              <View className="flex-row items-center">
                ...
                <Text className="ml-2 text-lg font-extrabold text-orange-900">Preparación</Text>
              </View>
              <View className="mt-3">
                {steps.map((step) => (
                  <View key={step.id} className="mb-4">
                    <View className="flex-row items-start">
                      <View className="mr-3 h-7 w-7 items-center justify-center rounded-full bg-orange-500">
                        <Text className="text-xs font-extrabold text-white">{step.step_number}</Text>
                      </View>
                      <View className="flex-1">
                        <Text className="text-sm font-extrabold text-orange-900">{step.title}</Text>
                        {step.duration_seconds ? (
                          <View className="mt-1 flex-row items-center">
                            <Ionicons name="timer-outline" size={12} color="#9a3412" />
                        ...
                        {step.tip ? (
                          <View className="mt-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2">
                            <View className="flex-row items-center">
                              ...
                              <Text className="ml-1 text-xs font-bold text-amber-800">Consejo de Mochi</Text>
                            </View>
                            <Text className="mt-0.5 text-xs font-semibold text-amber-700">{step.tip}</Text>
                          </View>
                        ) : null}
                      </View>
                    </View>
          ...
          <View className="mt-4 rounded-3xl border-2 border-orange-200 bg-white p-4">
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                ...
                <Text className="ml-2 text-lg font-extrabold text-orange-900">Mis notas</Text>
              </View>
              {!editingNotes && (
                <TouchableOpacity
                ...
                >
                  <Text className="text-xs font-extrabold text-orange-800">Editar</Text>
                </TouchableOpacity>
              )}
            </View>
            ...
            {editingNotes ? (
              <>
                ...
                <View className="mt-3 flex-row gap-3">
                  <TouchableOpacity
                    ...
                    onPress={() => { setEditingNotes(false); setNotesInput(recipe.personal_notes ?? '') }}
                  ...
                  >
                    {savingNotes
                      ...
                      : <Text className="text-center font-extrabold text-white">Guardar</Text>
                    }
                  </TouchableOpacity>
                </View>
              </>
              ...
              <Text className="mt-3 text-sm font-semibold text-slate-600">
                {recipe.personal_notes || 'Aún no tienes notas para esta receta.'}
              </Text>
            )}
          </View>
