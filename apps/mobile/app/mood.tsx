...
function getMoodLabel(value: number): string {
  return moodOptions.find((option) => option.value === value)?.label ?? 'Sin registro'
}
...
export function MoodScreen() {
  ...
  const loadMoodData = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'No se pudo cargar tu estado de ánimo')
      setTodayEntry(null)
      setRecentEntries([])
    } finally {
      setLoading(false)
  ...
  const handleSaveMood = async () => {
    ...
    try {
      ...
      showAlert({
        title: 'Error',
        message: err instanceof Error ? err.message : 'No se pudo guardar el estado de ánimo',
      ...
      <SafeAreaView className="flex-1 bg-orange-50">
        ...
        >
          ...
          <View className="mt-6 rounded-3xl border-2 border-orange-200 bg-white p-4">
            <Text className="text-2xl font-extrabold text-orange-900">Estado de ánimo</Text>
          ...
          {loading ? (
            <View className="mt-6 items-center rounded-3xl border-2 border-orange-200 bg-white p-6">
              ...
              <Text className="mt-3 text-sm font-semibold text-orange-800">Cargando registro de hoy...</Text>
            </View>
          ) : error ? (
            ...
            <>
              <View className="mt-6 rounded-3xl border-2 border-orange-200 bg-white p-4">
                <View className="flex-row items-center justify-between">
                  <Text className="text-lg font-extrabold text-orange-900">Hoy</Text>
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              <Text className="mt-2 text-sm font-extrabold text-slate-900">{option.label}</Text>
                            </View>
                          </TouchableOpacity>
                        )
                      })}
              ...
              <View className="mt-6 rounded-3xl border-2 border-orange-200 bg-white p-4">
                <Text className="text-lg font-extrabold text-orange-900">Últimos 7 días</Text>
                  ...
                  {last7Days.map((date) => {
                    ...
                    return (
                      <View key={date} className="items-center">
                        <View className={`h-8 w-8 rounded-full ${getMoodCircleClass(mood)}`} />
                        <Text className="mt-2 text-xs font-bold text-orange-700">{formatDateLabel(date)}</Text>
                      </View>
                    )
                  })}
                </View>
