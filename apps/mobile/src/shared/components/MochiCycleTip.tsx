...
const styleClassMap: Record<NonNullable<MochiCycleTipProps['style']>, string> = {
...
export function MochiCycleTip({
  ...
  const [loadedPreference, setLoadedPreference] = useState(!dismissible || !storageKey)
  ...
  return (
    <Animated.View entering={FadeInDown.duration(350)} className={className}>
      <MochiCharacter mood={personality.mochiMood} size={style === 'inline' ? 40 : 48} />
      <View className="ml-2 flex-1">
        <Text className="text-xs font-extrabold text-slate-800">{personality.phaseLabel}</Text>
        <Text className="mt-0.5 text-xs font-semibold text-slate-700">{tip}</Text>
      </View>
      {dismissible && (
        <TouchableOpacity onPress={() => void handleDismiss()} className="ml-2 rounded-full bg-white/80 p-1.5">
