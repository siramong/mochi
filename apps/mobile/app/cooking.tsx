    ...
    >
      <View className="flex-row items-start justify-between">
        <View className="flex-1 pr-3">
          ...
          {recipe.description ? (
            <Text className="mt-1 text-xs font-semibold text-orange-700" numberOfLines={2}>
              {recipe.description}
            </Text>
          ) : null}
        </View>
      ...
      <View className="mt-3 flex-row flex-wrap items-center gap-2">
        {totalTime > 0 && (
          <View className="flex-row items-center rounded-full bg-orange-100 px-3 py-1">
            ...
            <Text className="ml-1 text-xs font-bold text-orange-800">{totalTime} min</Text>
          </View>
        )}
        ...
        {recipe.servings > 0 && (
          <View className="flex-row items-center rounded-full bg-orange-100 px-3 py-1">
            ...
            <Text className="ml-1 text-xs font-bold text-orange-800">{recipe.servings} porc.</Text>
          </View>
        )}
        {recipe.cuisine_type ? (
          <View className="rounded-full bg-amber-100 px-3 py-1">
            <Text className="text-xs font-bold text-amber-800">{recipe.cuisine_type}</Text>
          </View>
        ) : null}
      </View>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      showAlert({
        ...
        message: err instanceof Error ? err.message : 'No se pudo generar la receta',
        ...
        <ScrollView className="flex-1 px-5 pt-12" showsVerticalScrollIndicator={false}>
          ...
          {loading ? (
            <View className="items-center py-12">
              ...
              <Text className="mt-3 text-sm font-semibold text-orange-700">Cargando recetas...</Text>
            </View>
          ) : error ? (
              ...
              <View className="max-h-[90%] rounded-t-3xl bg-white px-5 pb-10 pt-5">
                ...
                {generating ? (
                  <View className="items-center py-8">
                    ...
                    <ActivityIndicator className="mt-4" size="large" color="#f97316" />
                  </View>
                ) : (
                  <>
                    <Text className="text-xl font-extrabold text-orange-900">¿Qué quieres cocinar?</Text>
                    ...
                    <View className="mt-4 rounded-2xl border-2 border-orange-200 bg-orange-50 px-4 py-3">
                      ...
                      <Text className="text-xs font-semibold text-orange-600">
                        ...
                        • "algo dulce con plátano, huevo y avena"{'\n'}
                    ...
                    <View className="mt-5 flex-row gap-3">
                      ...
                      >
                        <Text className="text-center font-extrabold text-white">Generar</Text>
                      </TouchableOpacity>
                    </View>
                  </>
                )}
