...
export function RecipePlayerScreen() {
  ...
  const [cookSession, setCookSession] = useState<RecipeCookSession | null>(null)
    ...
    async function load() {
      ...
      try {
        const [recipeRes, stepsRes] = await Promise.all([
          supabase.from('recipes').select('*').eq('id', recipeId).eq('user_id', userId).single(),
          supabase
            .from('recipe_steps')
            .select('*')
        ...
        setError(err instanceof Error ? err.message : 'Error cargando la receta')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleAskMochi = async () => {
    ...
    try {
      setAsking(true)
      const answer = await askMochiWhileCooking(recipe.title, currentStep.title, question.trim())
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-orange-50">
        ...
        <Text className="mt-3 text-sm font-semibold text-orange-700">Preparando tu cocina...</Text>
      </SafeAreaView>
    )
  }
  ...
  if (error || !recipe) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-orange-50 px-6">
        ...
        <TouchableOpacity className="mt-6 rounded-2xl bg-orange-500 px-6 py-3" onPress={() => router.back()}>
  ...
  if (completed) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-orange-50 px-6">
        ...
        <Text className="mt-6 text-2xl font-extrabold text-orange-900">¡Buen provecho!</Text>
        ...
        <View className="mt-8 rounded-3xl border-2 border-orange-200 bg-white p-5">
          <Text className="text-center text-sm font-extrabold text-orange-900">¿Cómo quedó?</Text>
          <View className="mt-3 flex-row justify-center gap-2">
            {[1, 2, 3, 4, 5].map((star) => (
              <TouchableOpacity key={star} onPress={() => void handleSaveRating(star)}>
                <Ionicons
          ...
          {rating > 0 && (
            <Text className="mt-2 text-center text-xs font-semibold text-orange-500">
              {rating === 5 ? '¡Obra maestra!' : rating === 4 ? '¡Muy rico!' : rating === 3 ? '¡Bien!' : rating === 2 ? 'Mejorable' : 'Hay que practicar'}
            </Text>
          )}
        </View>
        ...
        <SafeAreaView className="flex-1">
          ...
          <View className="flex-row items-center justify-between">
            <TouchableOpacity
              ...
              onPress={() => {
                showAlert({
                  ...
                  message: 'Tu progreso se guardará. Podrás continuar desde aquí cuando quieras.',
                  buttons: [
            ...
            <TouchableOpacity
              ...
              onPress={() => { setShowAsk(true); setMochiAnswer('') }}
            >
          ...
          <View className="mt-3 h-2 w-full overflow-hidden rounded-full bg-orange-100">
            <Animated.View style={progressStyle} className="h-2 rounded-full bg-orange-500" />
          </View>

          <Text className="mt-1.5 text-xs font-semibold text-orange-500" numberOfLines={1}>
            {recipe.title}
          </Text>
        </View>
            ...
            <>
                  ...
                  <View className="mt-3 flex-row flex-wrap gap-2">
                    ...
                    {currentStep.temperature ? (
                      <View className="flex-row items-center rounded-full bg-red-100 px-3 py-1">
                        <Ionicons name="thermometer-outline" size={13} color="#b91c1c" />
                        <Text className="ml-1 text-xs font-bold text-red-700">{currentStep.temperature}</Text>
                      </View>
                    ) : null}
                  </View>
                )}
                ...
                {currentStep.tip ? (
                  <View className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
                    <View className="flex-row items-center">
                      ...
                      <Text className="ml-1.5 text-xs font-extrabold text-amber-800">Consejo de Mochi</Text>
                    </View>
                    <Text className="mt-1 text-sm font-semibold text-amber-700">{currentStep.tip}</Text>
                  </View>
                ) : null}
              </View>
              ...
              {currentStep.duration_seconds ? (
                <View className="mt-4 rounded-3xl border-2 border-orange-200 bg-white p-5">
                  <Text className="text-center text-sm font-bold text-orange-700">Temporizador</Text>
                  ...
                  <View className="mt-4 flex-row justify-center gap-3">
                    ...
                    >
                      <View className="flex-row items-center justify-center gap-2">
                        ...
                        />
                        <Text className={`font-extrabold ${timerRunning ? 'text-orange-800' : 'text-white'}`}>
                          {timerRunning ? 'Pausar' : timerSeconds === 0 ? 'Reiniciar' : 'Iniciar'}
                        </Text>
                      </View>
                    </TouchableOpacity>
                  </View>
              ...
              {mochiAnswer ? (
                <View className="mt-4 rounded-3xl border-2 border-yellow-200 bg-yellow-50 p-4">
                  ...
                  <TouchableOpacity className="mt-3 self-end" onPress={() => setMochiAnswer('')}>
                    <Text className="text-xs font-bold text-yellow-600">Cerrar</Text>
                  </TouchableOpacity>
                </View>
              ) : null}
            </>
          ...
          <View className="flex-row gap-3">
            ...
            >
              <Text className="text-base font-extrabold text-white">
                {currentIndex + 1 >= steps.length ? '¡Listo, a comer!' : 'Siguiente paso'}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
            ...
            <View className="max-h-[90%] rounded-t-3xl bg-white px-5 pb-10 pt-5">
              ...
              <View className="flex-row items-center">
                ...
                <View className="ml-3 flex-1">
                  <Text className="text-base font-extrabold text-orange-900">Pregúntale a Mochi</Text>
              ...
              {mochiAnswer ? (
                <View className="mt-4 rounded-2xl border-2 border-yellow-200 bg-yellow-50 p-4">
                  <Text className="text-sm font-semibold text-yellow-900 leading-5">{mochiAnswer}</Text>
                </View>
              ) : null}
              ...
              >
                {asking ? (
                  <>
                    ...
                    <Text className="ml-2 font-extrabold text-white">Pensando...</Text>
                  </>
                ) : (
                  <>
                    <Ionicons name="sparkles" size={16} color="white" />
                    <Text className="ml-2 font-extrabold text-white">Preguntar</Text>
                  </>
                )}
              </TouchableOpacity>
              </ScrollView>
