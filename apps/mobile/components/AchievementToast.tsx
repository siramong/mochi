...
export function AchievementToast({ title, description, points, icon, onHide }: AchievementToastProps) {
  ...
  useEffect(() => {
    ...
    translateY.value = withSequence(
      withTiming(0, { duration: SLIDE_IN_MS, easing: Easing.out(Easing.cubic) }),
      withDelay(
        VISIBLE_MS,
        withTiming(-120, { duration: SLIDE_OUT_MS, easing: Easing.in(Easing.cubic) })
      )
    )
    ...
    >
      <View className="flex-row items-center rounded-2xl border-2 border-yellow-300 bg-yellow-50 px-4 py-3 shadow-md">
        ...
        <View className="flex-1">
          ...
          <Text className="mt-0.5 text-sm font-extrabold text-yellow-950" numberOfLines={1}>
            {title}
          </Text>
          <Text className="text-xs font-semibold text-yellow-700" numberOfLines={1}>
            {description}
          </Text>
        </View>
        ...
        <View className="ml-3 rounded-full bg-yellow-300 px-2.5 py-1">
          <Text className="text-xs font-extrabold text-yellow-900">+{points}</Text>
        </View>
      </View>
    </Animated.View>
  )
